...
var syntax = {
    twitter: {
        ...
        url: /\b((?:(?:https?):(?:\/{1,3}|[a-z0-9%])|www\w*\.)(?:(?:[^\s()<>]+|\([^\s()<>]+\))+(?:\([^\s()<>]+\)|[^`!()\[\]{};:'".,<>?«»“”‘’\s])|\/?))/i,
        ...
        switch (current.type) {
            ...
            case "url":
                ...
                var urlobj = url.parse(
                                  /^www/i.test( current.token ) 
                                      ? 'http://' + current.token 
                                      : current.token
                              );

                // cut text before ampersands that don't have corresponding questionmarks
                var m = /^([^\?]+)(&.*)$/.exec( urlobj.href );
                if ( m ) {
                    tokens = tokenize( m[2], syntax["twitter"], 'text' ).concat( tokens );
                    current.token = m[1];
                    tokens.unshift( current );
                    continue;
                }
                
                // domain names may not follow '=', '/' or '!'
                ...
                if ( m ) {
                    ...
                    tokens = tokenize( bits.slice(1).join( '.' + m[1] ), syntax["twitter"], 'text' ).concat( tokens );
                ...
                this.html += "<a href=\"" + urlobj.href + "\">" + current.token + "</a>";
                break;
            case "hashtag":
